...
**Matching is radius-bound, Uber-style.** Rather than always returning the nearest N vendors regardless of how far "nearest" actually is, `rankVendors()` searches an **expanding ring of radii** — `[5, 10, 20, 35, 60]` km — starting tight and widening only if nothing's found, same principle as how Uber's dispatch search widens its radius when no nearby driver exists. Nothing within even the widest (60km) ring means a genuine "no vendors nearby," not a match with someone on the other side of the metro area. This is symmetric by construction: since a vendor only ever gets offered a job through this same search, a vendor also never sees a request that's unreasonably far from them — no separate "vendor side" filter needed. The same cap is enforced again as a guard in `POST /api/jobs/:id/select-vendor` (`MAX_SEARCH_RADIUS_KM`), so a direct API call can't bypass the radius rule just because it skips the ranked list.
...
| GET | `/api/jobs/:id/nearby-vendors` | Ranked available + approved vendors within an expanding search radius (Uber-style, max 60km) |
